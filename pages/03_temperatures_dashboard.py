...
def load_data():
    ...
    temps_df = pd.read_csv(data_path, index_col="Country")  # TODO: Ex 3.1: Load the dataset using Pandas, use the data_path variable and set the index column to "RecordID"
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    plt.title(f"Temperature Readings for {', '.join(selected_cities)}")
    plt.xlabel("Date")
    ...
    plt.title(f"Temperature Histogram for {', '.join(selected_cities)}")
